...
jq 'map(select(.files == ["applicationset"] or .files == ["applicationset", "docs"])) | "| ApplicationSet | applicationset/ | \(map(.count) | add) | \(map(.reviewers) | flatten | group_by(.user) | map({user: .[0].user, count: (map(.count) | add)}) | sort_by(.count) | reverse | .[:3] | map("\(.user) (\(.count))") | join(", ")) |"' -r pr-files-counts.json >> proposal.md
